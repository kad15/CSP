...
//sprintf(buf,"%s %d",mess, rang);
  ...
  if (rang != 0) {
    MPI_Recv(buf, 4096, MPI_CHAR, rang - 1, 0, MPI_COMM_WORLD,MPI_STATUS_IGNORE);
...
// envoi du message au pc (rang+1) module le nb de pc=10
  ...
  MPI_Ssend(buf, 4096, MPI_CHAR, (rang + 1)%nb , 0, MPI_COMM_WORLD);

  if (rang == 0) 
  {
    MPI_Recv(buf, 4096, MPI_CHAR, nb - 1, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
    printf("Le process %d a recu   %d du process %d\n", rang, buf, nb - 1);
    printf(buf," :  buf contient  %s\n",buf);
  }
